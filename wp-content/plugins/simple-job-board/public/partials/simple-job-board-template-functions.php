...
if (!function_exists('sjb_get_the_apply_now_btn')) {
    ...
    function   sjb_get_the_apply_now_btn() {
        ...
        if(get_option('sjb_quick_apply') == 'enable-quick-apply' )
            $quick_apply = get_option('quick_apply_btn_text') != '' ? '<a href="javascript:void(0)" id="quick-apply-btn" class="btn btn-primary '. apply_filters('sjb-listing-button-class', esc_attr( '' )).'" job_id="'. $job_id .'">' . esc_html__( $qck_btn_text, 'simple-job-board') . ' </a>': '';
        
        $read_more_btn = '<a href="' . get_the_permalink() . '" class="btn btn-primary '. apply_filters('sjb-listing-button-class', esc_attr( '' )).'">' . esc_html__( $read_more_btn_text, 'simple-job-board')  .'</a>';
        if (FALSE !== get_option('job_post_layout_settings')) {
            $jobpost_layout_option = get_option('job_post_layout_settings');
            if ('job_post_layout_version_one' === $jobpost_layout_option)
                $read_more_btn = '<a href="' . get_the_permalink() . '" class="btn btn-primary '. apply_filters('sjb-listing-button-class', esc_attr( '' )).'">' . esc_html__( $read_more_btn_text, 'simple-job-board')  .'</a>';
            ...
            if ('job_post_layout_version_two' === $jobpost_layout_option)
                $read_more_btn = '<a href="' . get_the_permalink() . '" class="btn btn-primary '. apply_filters('sjb-listing-button-class', esc_attr( '' )).'">' . esc_html__( $apply_now_text, 'simple-job-board')  .'</a>';
        }
...
if (!function_exists('sjb_job_archives_title')) {
    ...
    function sjb_job_archives_title($title) {
        ...
        $job_archives_name = get_option('job_archives_name') ? get_option('job_archives_name') : $title;
