                ...
                if (NULL != $keys):
                    foreach ($keys as $key):
                        if (substr($key, 0, 7) == 'jobapp_'):
                            ...
                            echo '<div class="applicant-columns-div"><input type="radio" class="applicant-columns" name="[applicant_column]" ' . esc_attr( $is_applicant_column ) . '/><span>' . esc_html__('Expose in Applicant Listing', 'simple-job-board') . ' </span>&nbsp; ';
                            echo '</div><input type="hidden" class="jobapp-applicant-column" name="' .  esc_attr( $key ) . '[applicant_column]" value="' . esc_attr( $is_applicant_column ) . '"/>';
                            ...
                            if (isset($val['type']) && isset($val['option'])):
                                if (substr($jobapp_field_name, 0, 7) == 'jobapp_'):
                                    ...
                                    echo '<li class="' . esc_attr( $jobapp_field_name ) . '"><label class="sjb-editable-label" for="">' . esc_attr( $label ). '</label>'
                                        ...
                                        . '<select class="jobapp_field_type" name="' . esc_attr( $jobapp_field_name ) . '[type]">' . wp_kses( $fields, $allowed_tags ) . '</select>';
                                    ...
                                    echo '<input type="hidden" class="jobapp-applicant-column" name="' . esc_attr( $jobapp_field_name ). '[applicant_column]" value="' . esc_attr(  $is_applicant_column ) . '" /><li>';
                                endif;
                            endif;
                        endforeach;
                    endif;
