...
foreach( $options['options'] as $option_id => $option_val ){
    ...
    if( is_array( $option_val ) && isset( $option_val['disabled'] ) ){
        ...
        $select->text( '<option value="'.$option_id.'" '.( $selected ? 'selected="selected"' : '' ).' '.( $option_val['disabled'] ? 'disabled="disabled"' : '' ).'>'. $option_val['label'] .'</option>' );
        ...
        foreach( $option_val['options'] as $_option_id => $_option_val ){
            ...
            if( $_option_id === 'category_walker' ){
               ...
               $select->text( '<option value="' . $_option_id . '" '.( in_array( $_option_id, $val ) ? 'selected="selected"' : '' ).'>'. $_option_val .'</option>' );
            }
        }
        ...
        $select->text( '<option value="'.$option_id.'" '.( $selected ? 'selected="selected"' : '' ).'>'. $option_val .'</option>' );
    }

}

echo '<div class="bf-select-option-container ' . $container_class . '">';
echo $select->display();
echo '</div>';
